...
def handle_bar(context, bar_dict):
    ...
    if (g.total_positionprevious != total_position) or (g.t % g.tc == 0):
        ...
        # 2.净利润同比增长率（高成长）
        net_profit_growth_ratio_df = net_profit_growth_ratio(
            stock_list, "growth_net_profit_growth_ratio", last_date
        )
...
def market_cap(stocklist, factor, last_date):
    ...
    # 取数据
    df = get_fundamentals(
        query(valuation.symbol, valuation.market_cap).filter(
...
def PB(stocklist, factor, last_date):
    # 取数据
    df = get_fundamentals(
        query(valuation.symbol, valuation.pb).filter(valuation.symbol.in_(stocklist)),
        date=last_date,
    )
    df = df.set_index("valuation_symbol")
    # 绝对中位数法取极值
...
def MTM20(stocklist, factor):
    ...
    for stock in stocklist:
        ...
        s = pd.DataFrame(
            [(df1["close"][-1] - df1["close"][0]) / df1["close"][0]], index=[stock]
        )
        #    log.info(s)
        if "df" in locals():
            df = df.append(s)
...
def roe(stocklist, factor, last_date):
    # 取数据
    df = get_fundamentals(
        query(valuation.symbol, profit.roe_ths).filter(valuation.symbol.in_(stocklist)),
        date=last_date,
    )
    #    log.info(df)
    df = df.set_index("valuation_symbol")
...
def net_profit_growth_ratio(stocklist, factor, last_date):
    # 取数据
    df = get_fundamentals(
        query(valuation.symbol, growth.net_profit_growth_ratio).filter(
            valuation.symbol.in_(stocklist)
        ),
        date=last_date,
    )
...
def fun_highlimit(bar_dict, stock_list):
    return [
        ...
        if bar_dict[stock].open != bar_dict[stock].high_limit
    ]
