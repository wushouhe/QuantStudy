...
def fun_main(context, bar_dict):
    ...
    if flag:
        ...
        # 选股
        buy_list = fun_get_buy_list(
            context, bar_dict, df_all.copy(), context.stock_max_num, last_date
        )

        # 计算股票权重
        stock_weight = fun_cal_stock_weight(
            buy_list, context.lag, context.confidenceLevel, last_date, "ES"
        )
        print("stock_weight: ")
        print(stock_weight)
...
def fun_cal_growth_rate(bar_dict, stock_list, last_date):
    ...
    def fun_get_quarterly_net_profit(current_quarter, last_quarter, stock_list):
        q = query(income.symbol, income.net_profit).filter(
            income.symbol.in_(stock_list)
        )
        ...
        # 当前单季度净利润
        df["quarterly_net_profit"] = (
            df["total_net_profit"] - df["last_total_net_profit"]
        )
    ...
    # 获取财报季度日期
    q = query(income.symbol, income.stat_date).filter(income.symbol.in_(stock_list))
    ...
    df_growth["growth_rate"] = (
        df_growth["current_net_profit"] / df_growth["last_net_profit"] - 1
    ) * 100
...
def fun_get_buy_list(context, bar_dict, df_all, n, last_date):
    ...
    def fun_get_market_cap(df_selected, last_date):
        ...
        # 获取股票市值
        q = query(valuation.symbol, valuation.market_cap).filter(
    ...
    if len(df_selected) >= n:
        ...
        for i in range(n):
            print(
                str(df_selected["symbol"][i]) + ", PEG = " + str(df_selected["PEG"][i])
            )
            ...
            for stock in old_stock_list:
                if i < n:
                    ...
                    print(
                        ...
                        + str(
                            df_selected_old.loc[
                                df_selected_old["symbol"] == stock, "PEG"
                            ].values
                        )
                    )
                    i += 1
...
def fun_cal_position(context, bar_dict, stock_weight, bond_weight, position_ratio=1.0):
    ...
    def fun_get_portfolio_VaR(dailyReturns, alpha):
        ...
        z_score = {0.05: 1.65, 0.04: 1.75, 0.01: 2.33, 0.0001: 3.7}

        VaR = -(
            dailyReturns["portfolio_returns"].mean()
            - z_score[alpha] * dailyReturns["portfolio_returns"].std()
        )
    ...
    def fun_get_equity_value(
        bar_dict, stock_weight, risk_money, max_risk_money, alpha, position_ratio
    ):
        ...
        if portfolio_ES == 0:
            ...
            equity_value = min(
                risk_money / portfolio_VaR, max_risk_money / portfolio_ES
            )
    ...
    stock_value = 0
    if stock_weight:
        stock_value = fun_get_equity_value(
            bar_dict,
            stock_weight,
            risk_money,
            max_risk_money,
